...
def fbconnect():
    ...
    url = 'https://graph.facebook.com/v3.0/me?access_token=%s&fie'
    'lds=name,id,email' % token
    h = httplib2.Http()
    result = h.request(url, 'GET')[1]
    ...
    # Get user picture
    url = 'https://graph.facebook.com/v3.0/me/picture?access_tok'
    'en=%s&redirect=0&height=200&width=200' % token
    h = httplib2.Http()
    result = h.request(url, 'GET')[1]
    data = json.loads(result)
